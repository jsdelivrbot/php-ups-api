...
class Shipping extends Ups
{
    ...
    private function createConfirmRequest(
         ...
         **/
        if ($shipment->getItemizedpaymentInformation()) {
            $paymentNode = $shipmentNode->appendChild($xml->createElement('ItemizedPaymentInformation'));

            if ($shipment->getItemizedpaymentInformation()->getSprepaid()) {
                $snode = $paymentNode->appendChild($xml->createElement('ShipmentCharge'));
                $snode->appendChild($xml->createElement('Type','01'));
                $knode=$snode->appendChild($xml->createElement('BillShipper'));

                $billShipper = $shipment->getItemizedpaymentInformation()->getSprepaid()->getBillShipper();

                if (isset($billShipper) && $shipment->getItemizedpaymentInformation()->getSprepaid()->getBillShipper()->getAccountNumber()) {
                    $knode->appendChild($xml->createElement('AccountNumber', $shipment->getItemizedpaymentInformation()->getSprepaid()->getBillShipper()->getAccountNumber()));
                }
            }

            if ($shipment->getItemizedpaymentInformation()->getSfreightCollect()) {
                $snode = $paymentNode->appendChild($xml->createElement('ShipmentCharge'));
                $snode->appendChild($xml->createElement('Type','01'));
                $knode = $snode->appendChild($xml->createElement('BillReceiver'));

                if ($shipment->getItemizedpaymentInformation()->getSfreightCollect()->getAccountNumber()) {
                    $knode->appendChild($xml->createElement('AccountNumber', $shipment->getItemizedpaymentInformation()->getSfreightCollect()->getAccountNumber()));
                }

                if ($shipment->getItemizedpaymentInformation()->getSfreightCollect()->getBillReceiverAddress()) {
                    $addressNode = $knode->appendChild($xml->createElement('Address'));
                    $addressNode->appendChild($xml->createElement('PostalCode', $shipment->getItemizedpaymentInformation()->getSfreightCollect()->getBillReceiverAddress()->getPostalCode()));
                }
            }

            if ($shipment->getItemizedpaymentInformation()->getTprepaid()) {

                $tnode = $paymentNode->appendChild($xml->createElement('ShipmentCharge'));
                $tnode->appendChild($xml->createElement('Type','02'));
                $ktnode=$tnode->appendChild($xml->createElement('BillShipper'));

                $tbillShipper = $shipment->getItemizedpaymentInformation()->getTprepaid()->getBillShipper();

                if (isset($tbillShipper) && $shipment->getItemizedpaymentInformation()->getTprepaid()->getBillShipper()->getAccountNumber()) {
                    $ktnode->appendChild($xml->createElement('AccountNumber', $shipment->getItemizedpaymentInformation()->getTprepaid()->getBillShipper()->getAccountNumber()));
                }
            }
        }

        // var_dump($shipment->getItemizedpaymentInformation());
